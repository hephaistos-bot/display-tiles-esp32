...
void hardware_init(void) {
    ...
    // --- EXACT Hardware Reset Sequence from 8456bdc ---
    ESP_LOGI(TAG, "Performing Hardware Reset Sequence (Commit 8456bdc)...");
    ...
    // 1. Hold Reset Low (TP), Release Reset (LCD), Hold INT Low, DISP ON (0)
    ESP_ERROR_CHECK(gpio_set_level((gpio_num_t)TP_INT_PIN, 0));
    ESP_ERROR_CHECK(ch422g_write_output(CH422G_EXIO_LCD_RST)); // LCD_RST=1 (Released), TP_RST=0 (Active), DISP=0 (ON)
    vTaskDelay(pdMS_TO_TICKS(10));

    // 2. Release TP Reset, Keep INT Low
    ESP_ERROR_CHECK(ch422g_write_output(CH422G_EXIO_LCD_RST | CH422G_EXIO_TP_RST)); // LCD_RST=1, TP_RST=1, DISP=0 (ON)
    vTaskDelay(pdMS_TO_TICKS(10));
...
esp_err_t init_sd_card(void) {
    ...
    // DISP must be maintained low (ON)
    ch422g_write_output(CH422G_EXIO_SD_CS | CH422G_EXIO_LCD_RST | CH422G_EXIO_TP_RST); // High (Inactive)
    vTaskDelay(pdMS_TO_TICKS(50));
    ch422g_write_output(CH422G_EXIO_LCD_RST | CH422G_EXIO_TP_RST); // Low (Active)
    vTaskDelay(pdMS_TO_TICKS(50));
